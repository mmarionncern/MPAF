...
FakeRatio::FakeRatio(std::string cfg){
  ...
  */
  ...
  //  startExecution(cfg);
  initialize();
  _ofileDump=new ofstream("eventDumpFR.txt", ios::out | ios::trunc );
}
...
void FakeRatio::initialize(){
  ...
  */
  ...
  vector<string> veln({"HLT_FR_Ele8_CaloIdM_TrackIdM_PFJet30","HLT_FR_Ele17_CaloIdM_TrackIdM_PFJet30"});
  vector<string> veli({"HLT_FR_Ele8_CaloIdL_TrackIdL_IsoVL_PFJet30","HLT_FR_Ele17_CaloIdL_TrackIdL_IsoVL_PFJet30"}); //
  vector<string> vmun({"HLT_FR_Mu8", "HLT_FR_Mu17"});
  vector<string> vmui({"HLT_FR_Mu8_TrkIsoVVL", "HLT_FR_Mu17_TrkIsoVVL"});
  ...
  // tree branches containing hlt*l1 prescales per event
  // string TR_psbh_el_non[1] = {"HLT_BIT_HLT_Ele12_CaloIdM_TrackIdM_PFJet30_v_Prescale"};
  // string TR_psbh_el_iso[1] = {"HLT_BIT_HLT_Ele12_CaloIdL_TrackIdL_IsoVL_PFJet30_v_Prescale"};
  // string TR_psbh_mu_non[2] = {"HLT_BIT_HLT_Mu8_v_Prescale" ,
  //                             "HLT_BIT_HLT_Mu17_v_Prescale"};
  // string TR_psbh_mu_iso[2] = {"HLT_BIT_HLT_Mu8_TrkIsoVVL_v_Prescale" ,
  //                             "HLT_BIT_HLT_Mu17_TrkIsoVVL_v_Prescale"};

  vector<string> prescale_el_non({"HLT_BIT_HLT_Ele8_CaloIdM_TrackIdM_PFJet30_v_Prescale","HLT_BIT_HLT_Ele17_CaloIdM_TrackIdM_PFJet30_v_Prescale"});
  vector<string> prescale_el_iso({"HLT_BIT_HLT_Ele8_CaloIdL_TrackIdL_IsoVL_PFJet30_v_Prescale","HLT_BIT_HLT_Ele17_CaloIdL_TrackIdL_IsoVL_PFJet30_v_Prescale"});
  vector<string> prescale_mu_non({"HLT_BIT_HLT_Mu8_v_Prescale", "HLT_BIT_HLT_Mu17_v_Prescale"});
  vector<string> prescale_mu_iso({"HLT_BIT_HLT_Mu8_TrkIsoVVL_v_Prescale", "HLT_BIT_HLT_Mu17_TrkIsoVVL_v_Prescale"});
  ...
  vector<float> efflum_el_non({getCfgVarF("LUMINOSITY_ELE8"     , 1.0), getCfgVarF("LUMINOSITY_ELE17"    , 1.0)});
  vector<float> efflum_el_iso({getCfgVarF("LUMINOSITY_ELE8_ISO" , 1.0), getCfgVarF("LUMINOSITY_ELE17_ISO"    , 1.0)});
  vector<float> efflum_mu_non({getCfgVarF("LUMINOSITY_MU8"      , 1.0), getCfgVarF("LUMINOSITY_MU17"    , 1.0)});
  vector<float> efflum_mu_iso({getCfgVarF("LUMINOSITY_MU8_ISO"  , 1.0), getCfgVarF("LUMINOSITY_MU17_ISO"    , 1.0)});
  ...
  _vTR_psbh_el_non    = prescale_el_non; //Tools::toVector(TR_psbh_el_non);
  _vTR_psbh_el_iso    = prescale_el_iso; //Tools::toVector(TR_psbh_el_iso);
  _vTR_psbh_mu_non    = prescale_mu_non; //Tools::toVector(TR_psbh_mu_non);
  _vTR_psbh_mu_iso    = prescale_mu_iso; //Tools::toVector(TR_psbh_mu_iso);
  ...
  // trigger effective luminosities
  _vTR_efflum_el_non = efflum_el_non; //Tools::toVector(TR_efflum_el_non);
  _vTR_efflum_el_iso = efflum_el_iso; //Tools::toVector(TR_efflum_el_iso);
  _vTR_efflum_mu_non = efflum_mu_non; //Tools::toVector(TR_efflum_mu_non);
  _vTR_efflum_mu_iso = efflum_mu_iso; //Tools::toVector(TR_efflum_mu_iso);
  ...
  // fake ratio pt bins
  float FR_bins_pt_el[7]  = {10.0, 15.0, 25.0, 35.0, 50.0, 70.0, 120.0};
  float FR_bins_pt_mu[7]  = {10.0, 15.0, 25.0, 35.0, 50.0, 70.0, 120.0};
  ...
  cout<<" target luminosity : "<<_targetLumi<<endl;


  _isRA5=(_idS=="RA5")?true:false;
...
float 
FakeRatio::puw2016_vtx(int nVtx) { if (nVtx<40) return _puw2016_vtx[nVtx]; else return 0; }
...
void FakeRatio::run(){
  ...
  _denLepsIdx   .clear();
  _denElsIsoIdx    .clear();
  _denLepsIsoIdx   .clear();
  _denMusIdx    .clear();
  _isoLepsIdx   .clear();
  _numElsIdx    .clear();
  _numLepsIdx   .clear();
  _numElsIsoIdx    .clear();
  _numLepsIsoIdx   .clear();
  _numMusIdx    .clear();
  ...
  // cout<<"bb "<<_vc->get("run")<<endl;
  //if(_vc->get("isData") && _vc->get("run")>275783) return;
  // if(_vc->get("evt")!=1521941921) return; //1521941921 1537770761
  //  cout<<"coin"<<endl;
  counter("runsel");
  counter("runsel", kEwkSel);
  counter("runsel", kUcsSel);
  counter("runsel", kQcdSel);

  // prepare event selection
  collectKinematicObjects();
  //cout<<"cuicui "<<_nDenLeps<<"   "<<_nDenEls<<"  "<<_nDenMus<<endl;
  ...
  //cout<<" ca paaaaaaaaaaaaaaaaaaaasssssssssssseeee "<<endl;
}
...
void FakeRatio::defineOutput(){
  ...
  */ 
  ...
  registerVariable("ER_HT"              , 1000,  0.0, 1000.0, "H_{T} [GeV]"                            ); 
  registerVariable("ER_MET"             , 1000,  0.0, 1000.0, "#slash{E}_{T} [GeV]"                    );
  ...
  registerLepPlots(vER_leps, "MT"       ,   40,  0.0,  200.0, "M_{T} [GeV]"                  );
  registerLepPlots(vER_leps, "MT0"      ,   40,  0.0,  200.0, "M_{T} [GeV]"                  );
  registerLepPlots(vER_leps, "MT1"      ,   40,  0.0,  200.0, "M_{T} [GeV]"                  );
  //registerLepPlots(vER_leps, "Pt"       , 1000,  0.0, 1000.0, "P_T(lep) [GeV]"                       );

  // UCSX Electroweak-Enriched Region
  registerVariable("UR_HT"              , 1000,  0.0, 1000.0, "H_{T} [GeV]"                            ); 
  registerVariable("UR_MET"             , 1000,  0.0, 1000.0, "#slash{E}_{T} [GeV]"                    );
  registerLepPlots(vUR_leps, "MT"       ,   40,  0.0,  200.0, "M_{T} [GeV]"                  );
  registerLepPlots(vUR_leps, "MT0"      ,   40,  0.0,  200.0, "M_{T} [GeV]"                  );
  registerLepPlots(vUR_leps, "MT1"      ,   40,  0.0,  200.0, "M_{T} [GeV]"                  );

  // Electroweak-Enriched Region
  registerVariable("QR_HT"              , 1000,  0.0, 1000.0, "H_{T} [GeV]"                            ); 
  registerVariable("QR_MET"             , 1000,  0.0, 1000.0, "#slash{E}_{T} [GeV]"                    );
  registerVariable("QR_MET_small"       ,    1,  0.0,   20.0, "#slash{E}_{T} [GeV]"                    );
  registerVariable("QR_MET_large"       ,    1, 45.0,   80.0, "#slash{E}_{T} [GeV]"                    );
  ...
  registerLepPlots(vQR_els, "MapPt"     , _vFR_bins_pt_el .size()-1, _vFR_bins_pt_el ,
                                          _vFR_bins_eta_el.size()-1, _vFR_bins_eta_el, "p_{T}(e) [GeV]"                 , "#||{#eta}(e)"  ); 
  registerLepPlots(vQR_els, "MapPtJet"  , _vFR_bins_pt_el .size()-1, _vFR_bins_pt_el ,
                                          _vFR_bins_eta_el.size()-1, _vFR_bins_eta_el, "p_{T}Tjet closest to e) [GeV]"  , "#||{#eta}(e)"  ); 
  registerLepPlots(vQR_els, "MapPtCorr" , _vFR_bins_pt_el .size()-1, _vFR_bins_pt_el ,
                                          _vFR_bins_eta_el.size()-1, _vFR_bins_eta_el, "p_{T}^{corr}(e) [GeV]"      , "#||{#eta}(e)"  ); 
  registerLepPlots(vQR_mus, "MapPt"     , _vFR_bins_pt_mu .size()-1, _vFR_bins_pt_mu ,
                                          _vFR_bins_eta_mu.size()-1, _vFR_bins_eta_mu, "p_{T}(#mu) [GeV]"               , "#||{#eta}(#mu)"); 
  registerLepPlots(vQR_mus, "MapPtJet"  , _vFR_bins_pt_mu .size()-1, _vFR_bins_pt_mu ,
                                          _vFR_bins_eta_mu.size()-1, _vFR_bins_eta_mu, "p_{T}(jet closest to #mu) [GeV]", "#||{#eta}(#mu)"); 
  registerLepPlots(vQR_mus, "MapPtCorr" , _vFR_bins_pt_mu .size()-1, _vFR_bins_pt_mu ,
                                          _vFR_bins_eta_mu.size()-1, _vFR_bins_eta_mu, "p_{T}^{corr}(#mu) [GeV]"    , "#||{#eta}(#mu)"); 
  // Measurement Region
  registerVariable("MR_HT"                , 1000,  0.0, 1000.0, "H_{T} [GeV]"                            ); 
  registerVariable("MR_MET"               , 1000,  0.0, 1000.0, "#slash{E}_{T} [GeV]"                    );
  ...
  registerVariable("MR_JetPt"             , 1000,  0.0, 1000.0, "p_{T}(jet) [GeV]"                       );
  ...
  registerLepPlots(vMR_leps, "MT"         , 1000,  0.0, 1000.0, "M_{T} [GeV]"                  );
  registerLepPlots(vMR_leps, "Pt"         , 1000,  0.0, 1000.0, "p_{T}(lep) [GeV]"                       );
  registerLepPlots(vMR_leps, "PtJet"      , 1000,  0.0, 1000.0, "p_{T}(jet closest to lep) [GeV]"        );
  registerLepPlots(vMR_leps, "PtCorr"     , 1000,  0.0, 1000.0, "p_T^{corr}(lep) [GeV]"            );
  ...
  registerLepPlots(vMR_els , "PtCorrFR"   , _vFR_bins_pt_el .size()-1, _vFR_bins_pt_el , "p_{T}^{corr}(e) [GeV]" );
  registerLepPlots(vMR_mus , "PtCorrFR"   , _vFR_bins_pt_mu .size()-1, _vFR_bins_pt_mu , "p_{T}^{corr}(#mu) [GeV]" );
  ...
  registerLepPlots(vMR_els , "PtCorrFR1"  , _vFR_bins_pt_el .size()-1, _vFR_bins_pt_el , "p_{T}^{corr}(e) [GeV]" );
  registerLepPlots(vMR_mus , "PtCorrFR1"  , _vFR_bins_pt_mu .size()-1, _vFR_bins_pt_mu , "p_{T}^{corr}(#mu) [GeV]" );
  ...
  registerLepPlots(vMR_els , "PtCorrFRHI" , _vFR_bins_pt_el .size()-1, _vFR_bins_pt_el , "p_{T}^{corr}(e) [GeV]" );
  registerLepPlots(vMR_mus , "PtCorrFRHI" , _vFR_bins_pt_mu .size()-1, _vFR_bins_pt_mu , "p_{T}^{corr}(#mu) [GeV]" );
  ...
  registerLepPlots(vMR_els , "PtCorrFRLO" , _vFR_bins_pt_el .size()-1, _vFR_bins_pt_el , "p_{T}^{corr}(e) [GeV]" );
  registerLepPlots(vMR_mus , "PtCorrFRLO" , _vFR_bins_pt_mu .size()-1, _vFR_bins_pt_mu , "p_{T}^{corr}(#mu) [GeV]" );
  ...
  registerLepPlots(vMR_els, "MapPtJet"    , _vFR_bins_pt_el .size()-1, _vFR_bins_pt_el ,
                                            _vFR_bins_eta_el.size()-1, _vFR_bins_eta_el, "p_{T}(jet closest to e) [GeV]"  , "#||{#eta}(e)"); 
...
void FakeRatio::divideFRMap(string postpend){
  ...
  for(unsigned int i = 0; i < _numDS + _idxs.size(); ++i){
    if(i == (size_t)_idx_datacorrCERN) continue;
...
void FakeRatio::doEwkSub(){
  ...
  //cout<<_ewkSub<<"  "<<foundData<<"  "<<foundEwk<<endl;
...
vector<float> FakeRatio::getScalesETH(string obs, float lumi){
  ...
  //cout<<obs<<"  "<<h_data -> GetEntries() <<endl;
...
vector<float> FakeRatio::getScalesUCSX(string obs, float lumi){
  ...
  vector<float> scales = Tools::toVector(sc);
  //cout<<lumi<<"  "<<h_data -> GetEntries()<<endl;
  if(lumi                   == 0) return scales;
  if(h_data -> GetEntries() == 0) return scales;
  if(h_data -> GetEntries() < 10) return scales;
...
vector<float> FakeRatio::getScalesUCSXlite(string obs, float lumi){
  ...
  // fit EWK to QCD-subtracted-data in range [70,120]
  cout<<"integrals "<<h_data->Integral(h_data->GetXaxis()->FindBin(70),h_data->GetXaxis()->FindBin(120))
      <<"    "<<h_ewk->Integral(h_ewk->GetXaxis()->FindBin(70),h_ewk->GetXaxis()-> FindBin(120))<<endl;
...
void FakeRatio::modifyWeight() {
  ...
  */ 
  ...
  if(_sampleName.find("data")==std::string::npos && _sampleName.find("runs")==std::string::npos) {
    ...
    _vtxWeight = _dbm->getDBValue("puw",_vc->get("nTrueInt")); //_susyMod->getPuWeight(_vc->get("nVert"));//_dbm->getDBValue("puw",_vc->get("nTrueInt"));//puw2016_vtx(_vc->get("nVert"));//_vc->get("vtxWeight");
    ...
    _weight *= _vtxWeight*_vc->get("genWeight");
    ...
    //cout<<" _weight "<<_vc->get("genWeight")<<"  "<<_vtxWeight<<"  "<<_dbm->getDBValue("XS", sname)<<"  "<<_dbm->getDBValue("KF", sname)<<"   "<<1./nProc<<"  "<<_dbm->getDBValue("XS", sname) * _dbm->getDBValue("KF", sname)/nProc*_weight<<endl;

    double w=_dbm->getDBValue("XS", sname) * _dbm->getDBValue("KF", sname)/nProc * 1;//last number is lumi
    w*=_targetLumi;

    if(_ie==0) {
      cout<<_sampleName<<" : "<<_dbm->getDBValue("XS", sname)<<"  "<<_dbm->getDBValue("KF", sname)<<"   "<<nProc<<"  "<<_targetLumi<<"  -->  "<<w<<" // "<<_weight*w<<endl;
    }
...
void FakeRatio::registerLepVars(){
  ...
  for(unsigned int i = 0; i < _leps.size(); ++i){
    ...
    _vc->registerVar(_leps[i] + "_etaSc"                );
    ...
    _vc->registerVar(_leps[i] + "_isFO_Mini"            );
    _vc->registerVar(_leps[i] + "_isFOL_Mini"            );
    _vc->registerVar(_leps[i] + "_isFOT_Mini"            );
    _vc->registerVar(_leps[i] + "_isTight_Mini"         );
    _vc->registerVar(_leps[i] + "_isTightL_Mini"         );
    _vc->registerVar(_leps[i] + "_isTightT_Mini"         );
  }
...
void FakeRatio::registerVariable(string var, int nBin, float min, float max, string Xleg, bool isglb, bool prof, string type){
  ...
  _hm->addVariable(var + "_non", nBin, min, max, Xleg, isglb, prof, vector<string>(0,""), type);
  _hm->addVariable(var + "_iso", nBin, min, max, Xleg, isglb, prof, vector<string>(0,""), type);

  for(unsigned int i = 0; i < _vTR_lines_el_non[0].size() + _vTR_lines_mu_non[0].size(); ++i) {
    //cout<<" --> "<<var + "_non" + Tools::intToString(i)<<endl;   
    _hm->addVariable(var + "_non" + Tools::intToString(i), nBin, min, max, Xleg, isglb, prof, vector<string>(0,""), type);
  }
  for(unsigned int i = 0; i < _vTR_lines_el_iso[0].size() + _vTR_lines_mu_iso[0].size(); ++i)
    _hm->addVariable(var + "_iso" + Tools::intToString(i), nBin, min, max, Xleg, isglb, prof, vector<string>(0,""), type);
...
void FakeRatio::registerVariable(string var, int nBinX, float minX, float maxX, int nBinY, float minY, float maxY, string Xleg, string Yleg, bool isglb, bool prof, string type){
  ...
  _hm->addVariable(var + "_non", nBinX, minX, maxX, nBinY, minY, maxY, Xleg, Yleg, isglb, prof, vector<string>(0,""), vector<string>(0,""), type);
  _hm->addVariable(var + "_iso", nBinX, minX, maxX, nBinY, minY, maxY, Xleg, Yleg, isglb, prof, vector<string>(0,""), vector<string>(0,""), type);

  for(unsigned int i = 0; i < _vTR_lines_el_non[0].size() + _vTR_lines_mu_non[0].size(); ++i)
    _hm->addVariable(var + "_non" + Tools::intToString(i), nBinX, minX, maxX, nBinY, minY, maxY, Xleg, Yleg, isglb, prof, vector<string>(0,""), vector<string>(0,""), type);
  for(unsigned int i = 0; i < _vTR_lines_el_iso[0].size() + _vTR_lines_mu_iso[0].size(); ++i)
    _hm->addVariable(var + "_iso" + Tools::intToString(i), nBinX, minX, maxX, nBinY, minY, maxY, Xleg, Yleg, isglb, prof, vector<string>(0,""), vector<string>(0,""), type);
...
void FakeRatio::registerVariable(string var, int nBinX, vector<float> binsX, string Xleg, bool isglb, bool prof, string type){
  ...
  _hm->addVariable(var + "_non", nBinX, binsX, Xleg, isglb, prof, vector<string>(0,""), type);
  _hm->addVariable(var + "_iso", nBinX, binsX, Xleg, isglb, prof, vector<string>(0,""), type);

  for(unsigned int i = 0; i < _vTR_lines_el_non[0].size() + _vTR_lines_mu_non[0].size(); ++i)
    _hm->addVariable(var + "_non" + Tools::intToString(i), nBinX, binsX, Xleg, isglb, prof, vector<string>(0,""), type);
  for(unsigned int i = 0; i < _vTR_lines_el_iso[0].size() + _vTR_lines_mu_iso[0].size(); ++i)
    _hm->addVariable(var + "_iso" + Tools::intToString(i), nBinX, binsX, Xleg, isglb, prof, vector<string>(0,""), type);
...
void FakeRatio::registerVariable(string var, int nBinX, vector<float> binsX, int nBinY, vector<float> binsY, string Xleg, string Yleg, bool isglb, bool prof, string type){
  ...
  _hm->addVariable(var + "_non", nBinX, binsX, nBinY, binsY, Xleg, Yleg, isglb, prof, vector<string>(0,""), vector<string>(0,""), type);
  _hm->addVariable(var + "_iso", nBinX, binsX, nBinY, binsY, Xleg, Yleg, isglb, prof, vector<string>(0,""), vector<string>(0,""), type);

  for(unsigned int i = 0; i < _vTR_lines_el_non[0].size() + _vTR_lines_mu_non[0].size(); ++i)
    _hm->addVariable(var + "_non" + Tools::intToString(i), nBinX, binsX, nBinY, binsY, Xleg, Yleg, isglb, prof, vector<string>(0,""), vector<string>(0,""), type);
  for(unsigned int i = 0; i < _vTR_lines_el_iso[0].size() + _vTR_lines_mu_iso[0].size(); ++i)
    _hm->addVariable(var + "_iso" + Tools::intToString(i), nBinX, binsX, nBinY, binsY, Xleg, Yleg, isglb, prof, vector<string>(0,""), vector<string>(0,""), type);
...
void FakeRatio::setBinErrors(TH1* central, TH1* upper, TH1* lower){
  ...
  for(int b = 1; b <= central->GetXaxis()->GetNbins(); ++b) {
    for(int c = 1; c <= central->GetYaxis()->GetNbins(); ++c) {
...
void FakeRatio::subtractPlots(string lep, int idx, vector<float> scales, string postfix){
  ...
  for(unsigned int i = 0; i < obs.size(); ++i) {
    ...
    // central value and errors for ewk scale only
      ...
      //cout<<obs[i]<<" ===> "<<postfix + num<<"  "<<idx<<"  "<<scales[0]<<"  "<<e->Integral()<<"  "<<d->Integral()<<endl;
  ...
  for(unsigned int i = 0; i < _numDS; ++i){
    ...
    for(unsigned int j = 0; j < obs.size(); ++j) {
	  ...
	  //cout<<" -->> "<<iso<<"  "<<end<<"   "<<obs[j]<<endl;
          ...
          if(!end){ //FIXME
            // if(linesel.size() <= idx) factor = _targetLumi/effsmu[idx - linesel.size()];
            // else                      factor = _targetLumi/effsel[idx];
...
void FakeRatio::collectKinematicObjects(){
  ...
  */
  ...
  _nDenElsIso  = _denElsIso .size();
  _nDenLepsIso = _denLepsIso.size();
  _nDenMus  = _denMus .size();
  _nNumEls  = _numEls .size();
  _nNumLeps = _numLeps.size();
  _nNumElsIso  = _numElsIso .size();
  _nNumLepsIso = _numLepsIso.size();
  ...
  _susyMod -> cleanJets( &_denLeps, _goodJets, _goodJetsIdx, _bJets, _bJetsIdx, _lepJets, _lepJetsIdx, 30, 25);
...
void FakeRatio::collectLeptons(int idx, string branch){
  ...
  if(std::abs(_vc->get(branch + "_pdgId", idx)) == 11){		  
    c = Candidate::create(_vc->get(branch + "_pt", idx),
  				  ...
  				  0.0005);
    //cout<<" electron --->>> "<<denominatorElectronSelection(c, idx, branch)<<"  "<<numeratorElectronSelection(c, idx, branch)<<endl;
    if(denominatorElectronSelection(c, idx, branch)) {
      _denEls       .push_back( c );
      _denElsIdx    .push_back(pair<int, string>(idx, branch));
      _denLeps      .push_back( _denEls.back() );
      _denLepsIdx   .push_back(pair<int, string>(idx, branch));

      if(!_id[kElTrEmu] || _susyMod -> elHLTEmulSelIso(idx, SusyModule::kLooseHT, branch)) {

      // if(_vc->get("LepGood_isFOT_Mini",idx)) {
       	//cout<<" ----> is isolated lepton "<<branch<<endl;
       	_denElsIso       .push_back( c );
       	_denElsIsoIdx    .push_back(pair<int, string>(idx, branch));
       	//cout<<" ==> "<<
       	_denLepsIso      .push_back( _denElsIso.back() );
       	_denLepsIsoIdx   .push_back(pair<int, string>(idx, branch));
       	_isoLepsIdx.push_back(pair<int, string>(idx, branch));
       }

      //if(!_id[kElTrEmu] || _susyMod -> elHLTEmulSelIso(idx, SusyModule::kLooseHT, branch)) 
      // if(!_id[kElTrEmu] || _vc->get("LepGood_isFOT_Mini",idx) )
      // 	_isoLepsIdx.push_back(pair<int, string>(idx, branch));
    }
    if(numeratorElectronSelection(c, idx, branch)) {
      _numEls       .push_back( c );
      _numElsIdx    .push_back(pair<int, string>(idx, branch));
      _numLeps      .push_back( _numEls.back() );
      _numLepsIdx   .push_back(pair<int, string>(idx, branch));
      // if(!_id[kElTrEmu] || _vc->get("LepGood_isTightT_Mini",idx) )
      // 	_isoLepsIdx.push_back(pair<int, string>(idx, branch));
      if(!_id[kElTrEmu] || _susyMod -> elHLTEmulSelIso(idx, SusyModule::kLooseHT, branch) ) {
      // if(_vc->get("LepGood_isFOT_Mini",idx)) {
       	_numElsIso       .push_back( c );
       	_numElsIsoIdx    .push_back(pair<int, string>(idx, branch));
       	_numLepsIso      .push_back( _numElsIso.back() );
       	_numLepsIsoIdx   .push_back(pair<int, string>(idx, branch));
       	_isoLepsIdx.push_back(pair<int, string>(idx, branch));
       }
  ...
  // muons
    ...
    if(denominatorMuonSelection(c, idx, branch)) {
      ...
      _denLeps   .push_back( _denMus.back() );
      _denLepsIso   .push_back( _denMus.back() );
      _denLepsIdx.push_back(pair<int, string>(idx, branch));
      _denLepsIsoIdx.push_back(pair<int, string>(idx, branch));
      _isoLepsIdx.push_back(pair<int, string>(idx, branch));
    }
    if(numeratorMuonSelection(c, idx, branch)) {
      ...
      _numLeps   .push_back( _numMus.back() );
      _numLepsIso   .push_back( _numMus.back());
      _numLepsIdx.push_back(pair<int, string>(idx, branch));
    }
  }
}
...
bool FakeRatio::denominatorElectronSelection(Candidate* c, int elIdx, string branch){
  ...
  //cout<<c->pt()<<"  "<<_susyMod->elIdSelV2(c, elIdx, branch, SusyModule::kFakable, 
  //false, false, false)<<endl;
  if(_isRA5) {
    if(!_susyMod->elIdSelV2(c, elIdx, branch, SusyModule::kFakable, 
			    false, false, false) ) return false;
  } else {
    if(!makeCut( _susyMod -> elIdSel(c, elIdx, _id[kElIdWP], SusyModule::kLoose, (_id[kElChCut]>0), false, false, branch), "electron id", "=", kDenEls)) return false;
    if(!makeCut( _susyMod -> elHLTEmulSel(elIdx, false, branch, _isRA5)                                                        , "non-iso emu", "=", kDenEls)) return false;
...
bool FakeRatio::denominatorMuonSelection(Candidate* c, int muIdx, string branch){
  ...
  // cout<<c->pt()<<"  "<<_susyMod->muIdSelV2(c, muIdx, branch, SusyModule::kFakable,
  // 					   false, false)<<endl;
  if(_isRA5) {
    if(!_susyMod->muIdSelV2(c, muIdx, branch, SusyModule::kFakable,
			    false, false) ) return false;
  } 
  else {
    if(!makeCut( _susyMod -> muIdSel(c, muIdx, _id[kMuIdWP], (_id[kMuChCut]>0), false, false, branch), "muon id", "=", kDenMus)) return false;
  }
...
bool FakeRatio::numeratorElectronSelection(Candidate* c, int elIdx, string branch){
  ...
  if(_isRA5) {
    if(!_susyMod->elIdSelV2(c, elIdx, branch, SusyModule::kTight,
			    false, true, false) ) return false;
  } 
  else {
    if(!makeCut( _susyMod -> elIdSel     (c, elIdx, _id[kElIdWP ], SusyModule::kLoose, (_id[kElChCut]>0), false, false, branch), "electron id ", "=", kNumEls)) return false;
    ...
    if(!makeCut( _susyMod -> elHLTEmulSel(elIdx, false, branch, _isRA5)                                                         , "electron emu", "=", kNumEls)) return false;
...
bool FakeRatio::numeratorMuonSelection(Candidate* c, int muIdx, string branch){
  ...
  if(_isRA5) {
    if(!_susyMod->muIdSelV2(c, muIdx, branch, SusyModule::kTight,
			    true, false) ) return false;
  }
  else {
    if(!makeCut( _susyMod -> muIdSel    (c, muIdx, _id[kMuIdWP ], (_id[kMuChCut] > 0), false, false, branch), "muon id"  , "=", kNumMus)) return false;
...
bool FakeRatio::baseSelection(){
  ...
  // lepton multiplicity
  if(!makeCut<int>( _nDenLeps, 1 , "=" , "lepton multiplicity and flavor"  )) return false; //cout<<" piou "<<endl;
  if     (_sampleName.find("datael") != std::string::npos){ 
    if(!makeCut<int>( _nDenEls           , 1 , "=" , "electron multiplicity and flavor")) return false; //cout<<" piou1 "<<endl;
  }
  else if(_sampleName.find("datamu") != std::string::npos){ 
    if(!makeCut<int>( _nDenMus           , 1 , "=" , "muon multiplicity and flavor"    )) return false; //cout<<" piou2 "<<endl;
  }

  //QCD muon samples
  if     (_sampleName.find("Mu15") != std::string::npos && 
	  !makeCut<float>(_denLeps[0] -> pt(), 15, ">" , "qcd mu15 binning (pt > 15)"    )) { return false; //cout<<" piou3 "<<endl;
  }
  else if(_sampleName.find("Mu5")  != std::string::npos &&
	  !makeCut<float>(_denLeps[0] -> pt(), 15, "<" , "qcd mu5 binning (pt < 15)"     )) { return false; //cout<<" piou4 "<<endl;
  }

  // jet multiplicity
  if(!makeCut<int>( _nJets             ,  1, ">=", "jet multiplicity"              )) return false; //cout<<" piou5 "<<endl;

  // triggers per FO
  if(!makeCut(triggerSelection()    , "trigger selection"     , "="                )) return false; //cout<<" piou6 "<<endl;
...
bool FakeRatio::ucsxEwkSelection(){
  ...
  // MET INVERTED! 
  if(!makeCut<float>( _met->pt() , 20.0, ">" , "MET selection"                 , 0, kUcsSel)) return false;
  //float mt=KineUtils::M_T(_denLeps[0]->pt(), _met->pt(), _denLeps[0]->phi(), _met->phi());
  //if(!makeCut<float>(mt, 70, "[]", "MT Sel", 120, kUcsSel)) return false;

  
  //if(!makeCut<float>( _met->pt() , 50.0, ">" , "MET selection"                 , 0, kUcsSel)) return false;
  //cout<<" pass event "<<endl;
  return true;
...
bool FakeRatio::mrSelection(){
  ...
  float mt=KineUtils::M_T(_denLeps[0]->pt(), _met->pt(), _denLeps[0]->phi(), _met->phi());
  //cout<<" MT "<<_denLeps[0]->pt()<<"  "<<_met->pt()<<"  "<<_denLeps[0]->phi()<<"  "<<_met->phi()<<" ---> "<<mt<<endl;
...
bool FakeRatio::triggerSelection(){ 
  ...
  if     (std::abs(_denLeps[0]->pdgId()) == 11) {
    ...
    if(etabin > -1 && ptbin > -1){
      vector<string> lines_non = _vTR_lineperpteta_mu_non[etabin][ptbin];
      vector<string> lines_iso = _vTR_lineperpteta_mu_iso[etabin][ptbin];
      //cout<<_denLeps[0]->eta()<<"  "<<_denLeps[0]->pt ()<<"  "<<lines_non.size()<<"  "<<lines_iso.size()<<endl;
      trlines = Tools::insertIntoVectorS(trlines, lines_non);
      trlines = Tools::insertIntoVectorS(trlines, lines_iso);
    }
  }
  //cout<<" --> "<<trlines.size()<<endl;
  if(trlines.size() > 0){
    for(unsigned int i = 0; i < trlines.size(); ++i){
      //cout<<"   ++> "<<Tools::trim(trlines[i])<<" >>>>> "<<_isoLepsIdx.size()<<"   "<<_denLepsIdx.size()<<endl;
      if(Tools::trim(trlines[i]) != ""){
        bool testiso = false;
        if(trlines[i].find("IsoVL") != std::string::npos || trlines[i].find("IsoVVL") != std::string::npos) 
          testiso = true;
	//cout<<Tools::trim(trlines[i])<<"  <=> "<<_vc->get(Tools::trim(trlines[i]))<<endl;
        if(!_isData || _vc->get(Tools::trim(trlines[i])) == 1) {
          if(testiso || _isoLepsIdx.size()==1){
...
void FakeRatio::fillEventPlots(){
  ...
  */
  ...
  for(unsigned int i = 0; i < _exts.size(); ++i){
    bool isIso=(_exts[i].substr(0,3)=="iso");
    //cout<<" weight check "<<i<<"   "<<_exts[i]<<"  "<<_weight <<"  "<<_trws[i]<<endl;
    ...
    fill("MR_NumDenEls_"  + _exts[i], (isIso?_denElsIso.size():_denEls.size()), _weight * _trws[i]);
    fill("MR_NumDenLeps_" + _exts[i], (isIso?_denLepsIso.size():_denLeps.size()), _weight * _trws[i]);
    fill("MR_NumDenMus_"  + _exts[i], _denMus .size()     , _weight * _trws[i]);
    fill("MR_NumJets_"    + _exts[i], _nJets              , _weight * _trws[i]);
    fill("MR_NumNumEls_"  + _exts[i], (isIso?_numElsIso.size():_numEls.size()), _weight * _trws[i]);
    fill("MR_NumNumLeps_" + _exts[i], (isIso?_numLepsIso.size():_numLeps.size()), _weight * _trws[i]);
...
void FakeRatio::fillEwkLepPlots(string prepend) { //, Candidate * lep, int lepIdx, string branch, int wp){
  ...
  */  
  ...
  for(unsigned int i = 0; i < _exts.size(); ++i){
    bool isIso=(_exts[i].substr(0,3)=="iso");
    float MT=KineUtils::M_T( (isIso?_denLepsIso[0]:_denLeps[0])->pt(), _met->pt(), (isIso?_denLepsIso[0]:_denLeps[0])->phi(), _met->phi());
    ...
    //cout<<i<<"  "<<prepend + "MT_"      + _exts[i]<<endl;
  }
...
//____________________________________________________________________________
void FakeRatio::fillFRMaps(string prepend, int lepIdx, string branch, int lepIdxIso, string branchIso, int wp){
  //cout<<" :"<<branch<<":"<<branchIso<<":"<<_exts.size()<<"  "<<_exts[0]<<endl;
  for(unsigned int i = 0; i < _exts.size(); ++i){
    bool isIso=(_exts[i].substr(0,3)=="iso");
    //cout<<i<<"  "<<_exts[i]<<"  "<<isIso<<endl;
    if(isIso) {
      lepIdx=lepIdxIso;
      branch=branchIso;
    }
...
void FakeRatio::fillFakeRatioMaps(){
  ...
  */
  ...
  for(unsigned int i = 0; i < _denEls.size(); ++i) { fillFRMaps("MR_DenEl", _denElsIdx[i].first, _denElsIdx[i].second, _denElsIsoIdx[i].first, _denElsIsoIdx[i].second, SusyModule::kTight ); }
  for(unsigned int i = 0; i < _denMus.size(); ++i) { fillFRMaps("MR_DenMu", _denMusIdx[i].first, _denMusIdx[i].second, _denMusIdx[i].first, _denMusIdx[i].second, SusyModule::kMedium); }
  for(unsigned int i = 0; i < _numEls.size(); ++i) { fillFRMaps("MR_NumEl", _numElsIdx[i].first, _numElsIdx[i].second,_numElsIsoIdx[i].first, _numElsIsoIdx[i].second, SusyModule::kTight );
                                                     fillFRMaps("MR_RatEl", _numElsIdx[i].first, _numElsIdx[i].second, _numElsIsoIdx[i].first, _numElsIsoIdx[i].second, SusyModule::kTight ); }
  for(unsigned int i = 0; i < _numMus.size(); ++i) { fillFRMaps("MR_NumMu", _numMusIdx[i].first, _numMusIdx[i].second, _numMusIdx[i].first, _numMusIdx[i].second, SusyModule::kMedium);
                                                     fillFRMaps("MR_RatMu", _numMusIdx[i].first, _numMusIdx[i].second, _numMusIdx[i].first, _numMusIdx[i].second, SusyModule::kMedium); }

}


//____________________________________________________________________________
void FakeRatio::fillLepPlots(string prepend, int lepIdx, string branch, int lepIdxIso, string branchIso, int wp){ //, Candidate * lep, int lepIdx, string branch, int wp){
  /*
    fills the control plots for leptons PER LEPTON
  */  
  ...
  for(unsigned int i = 0; i < _exts.size(); ++i){
    ...
    //cout<<i<<"  "<<_exts[i]<<"  "<<isIso<<"  "<<lepIdx<<"  "<<branch<<endl;
    ...
    //cout<<prepend<<"   "<<_exts[i]<<" ===> "<<_susyMod -> conePt(lepIdx, wp, branch)<<endl;
...
void FakeRatio::fillLeptonPlots(){
  ...
  */
  ...
  for(unsigned int i = 0; i < _denEls.size(); ++i) { fillLepPlots("MR_DenEl"  , _denElsIdx[i].first, _denElsIdx[i].second, _denElsIsoIdx[i].first, _denElsIsoIdx[i].second, SusyModule::kTight ); }
  for(unsigned int i = 0; i < _denMus.size(); ++i) { fillLepPlots("MR_DenMu"  , _denMusIdx[i].first, _denMusIdx[i].second, _denMusIdx[i].first, _denMusIdx[i].second, SusyModule::kMedium); }
  for(unsigned int i = 0; i < _numEls.size(); ++i) { fillLepPlots("MR_NumEl"  , _numElsIdx[i].first, _numElsIdx[i].second, _numElsIsoIdx[i].first, _numElsIsoIdx[i].second, SusyModule::kTight ); }
  for(unsigned int i = 0; i < _numMus.size(); ++i) { fillLepPlots("MR_NumMu"  , _numMusIdx[i].first, _numMusIdx[i].second, _numMusIdx[i].first, _numMusIdx[i].second,SusyModule::kMedium); }
...
void FakeRatio::fillQcdFakeRatioMaps(){
  ...
  */
  ...
  bool isIso=_denElsIso.size()==1;
  bool isIsoN=_numElsIso.size()==1;

  //cout<<" isIso ? "<<isIso<<endl;

  if(_met -> pt() < 20){
 

    for(unsigned int i = 0; i < _denEls.size(); ++i)   
      fillFRMaps("QR_small_DenEl", _denElsIdx[i].first, _denElsIdx[i].second, isIso?_denElsIsoIdx[i].first:0, isIso?_denElsIsoIdx[i].second:"", SusyModule::kTight );
    for(unsigned int i = 0; i < _denMus.size(); ++i)   
      fillFRMaps("QR_small_DenMu", _denMusIdx[i].first, _denMusIdx[i].second, _denMusIdx[i].first, _denMusIdx[i].second, SusyModule::kMedium);
    for(unsigned int i = 0; i < _numEls.size(); ++i) { 
      fillFRMaps("QR_small_NumEl", _numElsIdx[i].first, _numElsIdx[i].second, isIsoN?_numElsIsoIdx[i].first:0, isIsoN?_numElsIsoIdx[i].second:"", SusyModule::kTight );
      fillFRMaps("QR_small_RatEl", _numElsIdx[i].first, _numElsIdx[i].second, isIsoN?_numElsIsoIdx[i].first:0, isIsoN?_numElsIsoIdx[i].second:"", SusyModule::kTight ); }
    for(unsigned int i = 0; i < _numMus.size(); ++i) {
      fillFRMaps("QR_small_NumMu", _numMusIdx[i].first, _numMusIdx[i].second, _numMusIdx[i].first, _numMusIdx[i].second, SusyModule::kMedium);
      fillFRMaps("QR_small_RatMu", _numMusIdx[i].first, _numMusIdx[i].second, _numMusIdx[i].first, _numMusIdx[i].second, SusyModule::kMedium); }
  }

  if(_met -> pt() > 45 && _met -> pt() < 80){
    
    for(unsigned int i = 0; i < _denEls.size(); ++i) 
      fillFRMaps("QR_large_DenEl", _denElsIdx[i].first, _denElsIdx[i].second, isIso?_denElsIsoIdx[i].first:0, isIso?_denElsIsoIdx[i].second:"", SusyModule::kTight );
    for(unsigned int i = 0; i < _denMus.size(); ++i)   
      fillFRMaps("QR_large_DenMu", _denMusIdx[i].first, _denMusIdx[i].second, _denMusIdx[i].first, _denMusIdx[i].second, SusyModule::kMedium);
    for(unsigned int i = 0; i < _numEls.size(); ++i) { 
      fillFRMaps("QR_large_NumEl", _numElsIdx[i].first, _numElsIdx[i].second, isIsoN?_numElsIsoIdx[i].first:0, isIsoN?_numElsIsoIdx[i].second:"", SusyModule::kTight );
      fillFRMaps("QR_large_RatEl", _numElsIdx[i].first, _numElsIdx[i].second, isIsoN?_numElsIsoIdx[i].first:0, isIsoN?_numElsIsoIdx[i].second:"", SusyModule::kTight ); }
    for(unsigned int i = 0; i < _numMus.size(); ++i) { 
      fillFRMaps("QR_large_NumMu", _numMusIdx[i].first, _numMusIdx[i].second, _numMusIdx[i].first, _numMusIdx[i].second, SusyModule::kMedium);
      fillFRMaps("QR_large_RatMu", _numMusIdx[i].first, _numMusIdx[i].second, _numMusIdx[i].first, _numMusIdx[i].second, SusyModule::kMedium); }
  }
...
void FakeRatio::fillUcsxEwkLepPlots(string prepend){//, Candidate * lep, int lepIdx, string branch, int wp){
  ...
  */  
  ...
  //Candidate * mt = Candidate::create( lep, _met);0
  //float MT = _susyMod -> coneMt(lepIdx, lep, _met);
  //float MT2 = _susyMod -> Mt(_denLeps[0], _met, _denLepsIdx[0].first, -1, _denLepsIdx[0].second, "", (std::abs(_denLeps[0]->pdgId()) == 13?SusyModule::kMedium : SusyModule::kTight));
  //float MT=KineUtils::M_T(_denLeps[0]->pt(), _met->pt(), _denLeps[0]->phi(), _met->phi());
  //cout<<" ==> "<<MT<<"  "<<MT2<<endl;
  for(unsigned int i = 0; i < _exts.size(); ++i){
    bool isIso=(_exts[i].substr(0,3)=="iso");
...
void
FakeRatio::dumpEvents() {

  int run=_vc->get("run");
  int lumi=_vc->get("lumi");
  unsigned long event=_vc->get("evt");
  int id=_denLeps[0]->pdgId();
  float pt=_denLeps[0]->pt();
  float cpt=_susyMod->conePt(_denLepsIdx[0].first, SusyModule::kTight,_denLepsIdx[0].second);
  float eta=_denLeps[0]->eta();
  float mIso=_vc->get("LepGood_miniRelIso", _denLepsIdx[0].first);
  float ptRatio=_vc->get("LepGood_jetPtRatiov2", _denLepsIdx[0].first);
  float ptRel=_vc->get("LepGood_jetPtRelv2", _denLepsIdx[0].first);
  float MT=KineUtils::M_T(_denLeps[0]->pt(), _met->pt(), _denLeps[0]->phi(), _met->phi());
  float met=_met->pt();
  int isTight=_nNumLeps;

  *_ofileDump << Form("%1d%9d%12lu\t%+2d %2.4f\t%2.4f\t%2.4f\t%2.4f\t%2.4f\t%2.4f\t%2.4f\t%2.4f\t%1d\n",
		      run, lumi, event, id, pt, cpt, eta, mIso, ptRatio, ptRel,MT, met, isTight);
