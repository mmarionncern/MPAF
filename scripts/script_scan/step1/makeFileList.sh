...
for file in /pnfs/psi.ch/cms/trivcat/store/user/jhoss/samples/T5tttt/*.root
    do
        fdir=$(dirname "$file")
        fname=$(basename "$file")
        fname=${fname%.root}
        echo "$fdir      $fname" >> /mnt/t3nfs01/data01/shome/jhoss/analysis/MPAF/scripts/script_scan/step1/fileNamesT5ttttdeg
    done
