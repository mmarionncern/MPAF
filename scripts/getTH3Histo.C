...
void getTH3Histo(string iFile="scanScripts/filesScan", string oFile="histoScanT1tttt.root" ){

  ifstream ifile(iFile.c_str(), ios::in );
  ...
  TFile* ofile=new TFile(oFile.c_str(),"recreate");
  ofile->cd();
  h3->Write();
  ofile->Write();
  ofile->Close();
  cout<<" histogram merging done "<<endl;
}
